...
class Store
  class << self
    ...
    def add_to_list(name, additions)
      list_key = key(name)
      add_to_list_raw(list_key, additions) 
    end

    def add_to_list_raw(list_key, additions)
    ...
    def get_list(name)
      list_key = key(name)
      get_list_raw(list_key)
    end

    def get_list_raw(list_key)
...
class LunchPlaces < RandomAnswer
  private  

  def answers
    Store.get_list_raw("lunch-places")
  end  
end

class NewLunchPlace
  def initialize(lunch_place)
    @lunch_place = lunch_place
  end

  def run
    Store.add_to_list_raw("lunch-places", [@lunch_place])
    puts "I know about these lunch places: #{Store.get_list_raw("lunch-places").join(', ')}"
  end
end
...
class Help
  def run
    puts [COUNTERS, 'lunch', 'lunch-who?', 'img|image|pic|picture', '*cats*', 'harry', 'wizard', 'hagrid', 'where to lunch', 'new lunch place'].flatten.join(', ')
  end
end
...
command = 
  ...
  elsif /where to lunch/i =~ args[0..2].join(' ')
    LunchPlaces.new

  elsif /new lunch place:?/i =~ args[0..2].join(' ')
    NewLunchPlace.new(args[3..-1].join(' '))

  elsif args.first.downcase == 'help'
    Help.new
